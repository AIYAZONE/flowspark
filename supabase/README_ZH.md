...
本目录包含了初始化 Goal System 数据库所需的所有 SQL 脚本。请按照以下顺序在 Supabase SQL Editor 中执行这些脚本。
...
2. 进入项目的 **SQL Editor**。
3. 依次复制上述文件的内容并运行：
   - 先运行 `00_init_tables.sql`
   - 再运行 `01_rls_policies.sql`
   - 然后 `02_storage_setup.sql`
   - 最后 `03_functions_and_triggers.sql`
4. 执行完毕后，数据库即初始化完成。
