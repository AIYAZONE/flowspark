...
* NEXT_PUBLIC_SUPABASE_ANON_KEY：Supabase 项目匿名公钥（Anon Key）。

  * 获取路径：Supabase 控制台 → 项目 → Settings → API → anon public。仅用于公开环境，不可写入敏感数据。

  * 用途：客户端/服务端创建 Supabase 客户端所需的公开密钥。

* NEXT_PUBLIC_SITE_URL：应用的对外访问完整 URL（含协议），例如 https://goals.aiya.zone 或 https://<vercel-domain>.vercel.app。
...
* 注意事项：
  ...
  * 变更域名后应同步更新 NEXT_PUBLIC_SITE_URL 以保证重置回跳正确。
  
### 环境变量取值策略（Vercel）

* Production：`NEXT_PUBLIC_SITE_URL=https://<your-domain>` 指向正式域名

* Preview：推荐代码侧回退至 `https://${process.env.VERCEL_URL}`，无需手动为每次预览设置变量

* Development：`NEXT_PUBLIC_SITE_URL=http://localhost:3000`

* SITE_URL 计算示例（用于 Supabase 回跳）：
...
* 作用：保证在 Preview 部署下密码重置、邮箱确认等回跳链接始终有效

* 禁止：在客户端暴露 Service Role Key；Anon Key 仅用于公开可读/受限写的场景
...
* `redirectTo` 统一使用：`SITE_URL + '/reset'`，在 Preview 环境自动回跳至对应预览域名的 `/reset`

* Supabase 控制台：Settings → Authentication → URL Configuration

  * Site URL：设置为 `SITE_URL`

  * Redirect URLs：允许 `SITE_URL` 以及 Preview 通配域名（例如 `*.vercel.app`）

* 说明：注册邮箱确认与找回密码邮件均复用上述回跳策略

## 授权与数据安全（AuthZ & RLS）

* 规则：所有表（goals、actions 等）添加列 owner_id，RLS：用户仅能访问 owner_id = auth.uid() 的记录

* 读策略：USING owner_id = auth.uid()；写策略：WITH CHECK owner_id = auth.uid()
...
* 域名与环境
  ...
  * Vercel Project → Domains：绑定正式域名，同时保留默认 `*.vercel.app` 作为 Preview

  * 三环境变量：Production/Preview/Development；Preview 由 `VERCEL_URL` 提供域名

* 路由保护与中间件

  * 策略不变：未登录访问受保护路由跳转 `/login`；已登录访问公共认证页重定向 `/dashboard`

* 邮件与回跳

  * 在客户端构造 `redirectTo` 使用 `SITE_URL`，避免硬编码域名导致预览环境失效

* 日志与监控建议

  * 记录 `requestReset` 与 `reset` 成功/失败事件（不含敏感信息）

  * 为 Auth 关键路径设置轻量告警（失败率、响应时间），与运维文档联动
...
* 可选：删除账户（软删除，标记 deleted_at），后续安排数据保留与导出

## 个人资料（Profile）

* 表结构：user_profiles(id uuid 主键 = auth.user.id, name, avatar_url, locale, timezone, created_at, updated_at)
...
- 安全与合规策略：[12 安全与合规策略.md](file:///Users/brucewang/Documents/AIYA/goals/.trae/documents/运营与交付/12%20安全与合规策略.md)
- 支持与反馈流程：[14 支持与反馈流程.md](file:///Users/brucewang/Documents/AIYA/goals/.trae/documents/运营与交付/14%20支持与反馈流程.md)
...
* 表：user_profiles、goals、actions（均含 owner_id）

* 索引：owner_id 常用查询字段建立索引；created_at 排序索引
...
* 事件记录（可选）：last_login_at、profile 更新写入 updated_at；后续扩展审计日志表
...
* 邀请与团队多租户结构（projects/teams），以 owner_id+tenant_id 扩展 RLS

* 风控与安全：密码重复使用检测、常见泄露密码拦截

### 审计日志与异常登录检测（补充）

* 审计日志：记录登录、密码重置、资料更新等关键事件（不含敏感字段）

* 异常检测：短时间多次失败登录、非常规地区/设备登录提示复核

## 实施路径（MVP）

1. 完成与验证 RLS 策略（owner_id + USING/WITH CHECK）
2. /profile 增强：头像、时区、姓名校验与服务端动作
3. /reset 继续完善密码强度与错误映射（已完成基础版本）
4. 部署变量与邮件回调域名配置（NEXT_PUBLIC_SITE_URL、Supabase URL/Key）
