# FlowSpark 技术架构文档

```mermaid
graph TD
    A[User Browser] --> B[Next.js App Router]
    B --> C[Server Components]
    B --> D[Server Actions]
    
    subgraph "Frontend & BFF Layer (Next.js)"
        B
        C
        D
        I[AI SDK]
    end

    C --> E[Supabase SDK (SSR)]
    D --> E
    I --> J[LLM Provider]
    
    E --> F[Supabase Auth]
    E --> G[Supabase Database]
    E --> H[Supabase Storage]

    subgraph "Service Layer (Supabase)"
        F
        G
        H
        K[Vector Store (pgvector)]
    end
    
    subgraph "External Services"
        J[OpenAI / Anthropic]
    end
```

## 2. 技术栈描述

- **前端框架**: Next.js 16.1 (App Router)
- **核心库**: React 19.2
- **样式方案**: Tailwind CSS 4.0 + Radix UI (Headless UI) + Lucide React (Icons)
- **状态管理**: Zustand 5.0 (配合 Server State)
- **AI 集成**: Vercel AI SDK (Core + React)
- **后端服务**: Supabase (BaaS)
- **向量数据库**: Supabase pgvector (用于 RAG)
- **数据交互**: Server Actions + Server Components
- **国际化**: Custom i18n implementation (English/Chinese)
- **部署**: Vercel

## 3. 路由结构 (App Router)

| 路由 | 文件路径 | 用途 |
|------|----------|------|
| / | src/app/page.tsx | 落地页 |
| /login | src/app/login/page.tsx | 登录/注册 |
| /dashboard | src/app/(authenticated)/dashboard/page.tsx | 仪表盘 |
| /goals | src/app/(authenticated)/goals/page.tsx | 目标列表 |
| /goals/new | src/app/(authenticated)/goals/new/page.tsx | 创建目标 |
| /goals/[id] | src/app/(authenticated)/goals/[id]/page.tsx | 目标详情 |
| /today | src/app/(authenticated)/today/page.tsx | 今日行动 |
| /profile | src/app/(authenticated)/profile/page.tsx | 个人设置 |

## 4. 数据模型

### 4.1 核心实体关系

```mermaid
erDiagram
    USER ||--o{ GOAL : manages
    USER ||--o{ ACTION : tracks
    USER ||--o{ DAILY_SCORE : records
    USER ||--o{ GAMIFICATION_PROFILE : owns
    USER ||--o{ REWARD_INVENTORY : collects
    GOAL ||--o{ ACTION : contains

    GOAL {
        uuid id PK
        uuid user_id FK
        string title
        text description
        date start_date
        date end_date
        string priority "low|medium|high"
        string category
        string status "active|completed|abandoned"
        timestamp created_at
        timestamp updated_at
    }
    
    ACTION {
        uuid id PK
        uuid goal_id FK
        string title
        string type "core|maintenance|learning|review|rest"
        date start_date
        date end_date
        boolean completed
        timestamp created_at
        timestamp updated_at
    }

    DAILY_SCORE {
        uuid id PK
        uuid user_id FK
        date score_date
        integer score
    }

    GAMIFICATION_PROFILE {
        uuid user_id PK
        integer xp
        integer level
        integer current_streak
        integer max_streak
        jsonb preferences
    }

    REWARD_INVENTORY {
        uuid id PK
        uuid user_id FK
        string item_type
        jsonb item_metadata
        timestamp acquired_at
    }
```

### 4.2 数据库 Schema 更新说明

**Goals Table**
- 新增字段 `priority`: 目标优先级 (default: 'medium')
- 新增字段 `category`: 目标分类 (default: 'other')

**Actions Table**
- 字段迁移: `action_date` 逐步迁移至 `start_date`
- 新增字段 `end_date`: 支持跨天行动

**New: Gamification Tables (Phase 2)**
- `gamification_profile`: 存储用户等级、XP、连胜纪录。
- `reward_inventory`: 存储用户获得的虚拟奖励（徽章、道具）。

## 5. 核心交互机制 (Server Actions)

系统采用 Next.js Server Actions 处理数据变更，确保类型安全和前后端逻辑紧密结合。

### 5.1 目标管理 (`src/app/(authenticated)/goals/actions.ts`)
- `createGoal(formData)`: 创建新目标，支持优先级和分类设置
- `updateGoal(formData)`: 更新目标详情
- `deleteGoal(formData)`: 删除目标及其关联的所有行动

### 5.2 行动管理 (`src/app/(authenticated)/goals/actions.ts`)
- `createAction(formData)`: 创建行动，支持日期范围
- `updateAction(formData)`: 更新行动信息
- `deleteAction(formData)`: 删除指定行动

### 5.3 仪表盘交互 (`src/app/(authenticated)/dashboard/actions.ts`)
- `toggleAction(formData)`: 切换行动的完成状态 (Optimistic Update support in UI)
- `submitScore(formData)`: 提交每日评分 (Upsert logic)

## 6. 部署配置

### 6.1 环境变量
```bash
NEXT_PUBLIC_SUPABASE_URL=...
NEXT_PUBLIC_SUPABASE_ANON_KEY=...
OPENAI_API_KEY=... # For AI features
```

### 6.2 项目配置
- `next.config.ts`: Next.js 配置文件
- `tailwind.config.ts`: (v4 集成在 CSS 中或单独配置)
- `middleware.ts`: 处理路由保护和重定向
  - 语言选择：通过 Cookie `NEXT_LOCALE` 设置与读取（默认 en，Accept-Language 含 zh 时为 zh）
  - i18n 加载：`getDictionary(locale?)`，字典类型由 `typeof en.json` 推断，保证类型安全
  - 时区工具：`getUserTimezone/getTodayInTZ/toLocaleDateStringTZ`，统一“今日”逻辑与展示

## 错误处理与监控告警

- 错误分层：用户错误（表单校验）、权限错误（RLS/未登录）、系统错误（DB/网络）
- 反馈策略：面向用户的可读消息；开发日志中记录错误代码与上下文
- 监控：认证成功率、密码重置成功率、关键页面 LCP；超阈触发告警
- 数据迁移与备份：Schema 变更脚本与回滚计划；数据库定期备份与恢复演练

## 关联文档

- API 契约与数据字典：[08 API与数据字典.md](file:///Users/brucewang/Documents/AIYA/goals/.trae/documents/技术/08%20API与数据字典.md)
- 运维与监控：[09 运维与监控.md](file:///Users/brucewang/Documents/AIYA/goals/.trae/documents/技术/09%20运维与监控.md)
