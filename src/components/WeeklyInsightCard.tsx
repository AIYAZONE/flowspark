...
export function WeeklyInsightCard({ dict, locale, insight, compact = false }: WeeklyInsightCardProps) {
  ...
  return (
    <Card className="overflow-hidden border-primary/12 bg-linear-to-br from-primary/5 via-card to-card">
        ...
        {!insight ? (
          ...
          <>
            ...
            <div className="rounded-2xl border border-primary/12 bg-background/80 p-4">
              <div className="inline-flex items-center gap-2 text-xs font-medium uppercase tracking-[0.16em] text-primary/80">
                <Sparkles className="h-3.5 w-3.5" />
                <span>{locale === 'zh' ? '系统摘要' : 'System summary'}</span>
              </div>
              <div className="mt-3 text-sm leading-7">{insight.output.summary}</div>
            </div>
            <div className={`gap-3 rounded-2xl border border-border/50 bg-muted/20 p-3 text-sm ${compact ? 'space-y-2' : 'grid md:grid-cols-3'}`}>
            ...
            <div className="space-y-1">
              ...
              <div className="rounded-2xl border border-border/50 bg-background/75 p-4 text-sm leading-7">{insight.output.recommendation}</div>
            </div>
            <div className="rounded-2xl border border-dashed border-border/60 bg-muted/20 px-3 py-2 text-xs leading-5 text-muted-foreground">
